...
import threading
import time
import socket  # Required for direct port scanning
...
def check_sip_ports(ip, ports=[5060, 5927], timeout=2):
    ...
    Returns True if at least ONE of the ports accepts the connection.
    """
    for port in ports:
        try:
            with socket.create_connection((str(ip), port), timeout=timeout):
                return True  # The port is open and listening!
        except (socket.timeout, ConnectionRefusedError, OSError):
            continue  # Try the next port if this one fails
    return False  # Both ports are closed or timing out
...
def process_alert_queue():
    ...
    while not alert_queue.empty():
        ...
        try:
            ...
            # --- FIXED: MULTI-LAYER ALTERNATE SELECTOR STRATEGY FOR WHATSAPP UPDATES ---
            box = None
            xpaths_to_try = [
            ...
            for path in xpaths_to_try:
                try:
                    ...
                    continue  # Failover to next structural path definition
...
def check_device(device):
    try:
        ...
        # 2. Deep port scanning (Only run on individual extensions, NOT on the main GrandStream UCM)
        ...
        return {"name": name, "ip": ip, "phone": phone, "type": dtype, "online": is_online, "status_flag": status_flag}
    except Exception as e:
    ...
    try:
        while True:
            try:
                ...
                for r in results:
                    ip, name, phone, dtype, online, status_flag = r["ip"], r["name"], r["phone"], r["type"], r["online"], r["status_flag"]

                    # --- SCRIPT INITIALIZATION ALERT LOGIC ---
                    if ip not in device_state:
                        device_state[ip] = online
                        
                        if not online:
                            failure_count[ip] = FAILURE_THRESHOLD  # Set directly to threshold
                            print(f"INIT 🚨 {name} ({ip}) is ALREADY {status_flag} at startup! Queueing alert...")
                            ...
                            failure_count[ip] = 0
                            print(f"INIT 💚 {name} ({ip}) → ONLINE")
                        continue

                    # --- NORMAL RUNNING SCRIPT LOGIC ---
                    if online:
                        ...
                        failure_count[ip] += 1
                        print(f"⚠️ FAIL {name} ({failure_count[ip]}/{FAILURE_THRESHOLD}) - Reason: {status_flag}")

                        if failure_count[ip] == FAILURE_THRESHOLD and device_state[ip]:
                            device_state[ip] = False
                            
                            # INTERCEPTION: If a desk extension drops, check if the core GrandStream server is alive via Ping ONLY
                            if ip.startswith("192.168.3.") and ip != "192.168.3.2":
                                print(f"🔍 Extension down ({name}). Checking GrandStream Main Server status...")
                                main_server_online = ping("192.168.3.2")
                                
                                if not main_server_online:
                                    print("🚨 CRITICAL: GrandStream Main Server is completely unreachable! Overriding with Server Hang alert.")
                                    alert_queue.put({
                                    ...
                                    continue # Drops the individual phone alert to avoid spamming text messages
                            
                            # CHOOSE CORRECT WHATSAPP MESSAGE
                            if status_flag == "SIP REGISTRATION FAILED":
                            ...
                            print(f"🚨 Queueing notification text for {name}")
